...
// const route = useRoute()
const posts = await useAsyncData(() => queryCollection('content').order('date', 'DESC').all())

const { data: sections } = await useAsyncData('search-sections', () => {
  return queryCollectionSearchSections('content')
})

// const allBlog = await useAsyncData('/', () => {
...
const articles = posts.data.value
...
const suggestions = computed(() => {
  ...
  return {
    ...
    articles: sections.value.filter(article =>
      article.title.toLowerCase().includes(query),
    ).map((item) => {
      return { ...item, longName: `${item.titles.join(' > ')} > ${item.title}` }
    }),
  }
})
...
<template>
  <div class="mx-auto max-w-[800px] p-8 px-4 dark:bg-gray-900">
        ...
        <Transition name="suggestion">
          ...
          >
            ...
            <div v-if="suggestions.articles.length > 0">
              ...
              >
                <span class="truncate text-sm">{{ article.longName }}</span>
              </div>
            </div>
          </div>
        </Transition>
    ...
    <div class="overflow-y-auto pr-2">
      ...
      >
        <div class="cursor-my-pointer flex flex-col items-start gap-2 md:flex-row md:items-center" @click="navToContent(article.path)">
